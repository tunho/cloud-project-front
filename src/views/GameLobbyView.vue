...
function confirmBetting() {
  ...
  closeBettingModal();
  // 🔥 [FIX] Pass gameType to MatchingView
  router.push({ 
      path: "/matching", 
      query: { 
          bet: betAmount.value.toString(),
          game: props.gameType 
      } 
  }); 
}
...
function goCustomMatch() {
  router.push({ path: "/custom-match", query: { game: props.gameType } });
}
...
socket.on("match:success", (data) => {
  ...
  // Redirect based on game type (though backend should handle room creation)
  // Actually, match:success usually means room is ready.
  // We need to know where to go.
  // Assuming room creation sets gameType, CustomRoomView handles redirect?
  // No, MatchingView handles redirect.
  // This listener might be redundant here if MatchingView handles it.
  // But DavinciHomeView had it.
  // Let's keep it but make it smart.
  // Wait, DavinciHomeView redirects to /room/:roomId/play directly.
  // If it's Omok, we should go to /room/:roomId/omok.
  // But we don't know gameType from data unless backend sends it.
  // Backend `match_success` event sends `roomId` and `players`.
  // We should probably rely on MatchingView for the actual transition.
  // But if we are here, we are not in MatchingView yet.
  // Ah, confirmBetting pushes to /matching. So this listener is not needed here?
  // DavinciHomeView had it... maybe for some other flow?
  // Let's comment it out or remove it to avoid confusion, as we go to MatchingView.
